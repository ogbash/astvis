...
class TagGraphTest(unittest.TestCase):
    ...
    def testAdd(self):
        ...
        self.assertEquals(len(tg._tags), 2)
    ...
    def testRemove(self):        
        ...
        tg=taggraph.TagGraph(gtv, gsv)
        tg.addTag('cool', graph[3], 'cool')
        tg.addTag('cool2', graph[4], 'cool')

        tg.removeTag('cool', graph[3], 'cool')

        self.assertEquals(len(tg._tags), 1)
        self.assertEquals(len(tg._inducedTags[graph[3]]), 1)
        self.assertEquals(len(tg._inducedTags[graph[1]]), 1)

        tg.removeTag('cool2', graph[4], 'cool')

        self.assertEquals(len(tg._tags), 0)
        self.assertEquals(len(tg._inducedTags), 0)
